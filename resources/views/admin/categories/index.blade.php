...
<div>
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:24px;gap:16px;">
        <div>
            <h1 style="font-size:24px;font-weight:700;color:#0f172a;letter-spacing:-0.02em;margin-bottom:4px;">Manajemen Kategori</h1>
            <p style="font-size:14px;color:#475569;">Kelola daftar kategori yang tersedia untuk acara.</p>
        </div>
    ...
    <div style="background:#fff;border:1px solid #f1f5f9;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px 0 rgba(15,23,42,.03);">
            ...
            <table style="width:100%;border-collapse:collapse;">
                <thead>
                    <tr style="background:#f8fafc;border-bottom:1px solid #f1f5f9;">
                        ...
                        <th style="padding:12px 20px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;text-align:left;">Nama Kategori</th>
                        <th style="padding:12px 20px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;text-align:left;">Jumlah Event</th>
                        ...
                        <th style="padding:12px 20px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;text-align:left;">Diperbarui</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #f1f5f9;transition:background 150ms;"
                        ...
                        <td style="padding:14px 20px;font-size:14px;color:#94a3b8;">{{ $categories->firstItem() + $index }}</td>
                        <td style="padding:14px 20px;font-size:14px;font-weight:600;color:#1e293b;">{{ $category->name }}</td>
                        ...
                        <td style="padding:14px 20px;font-size:13px;color:#94a3b8;">{{ $category->created_at->format('d M Y') }}</td>
                        <td style="padding:14px 20px;font-size:13px;color:#94a3b8;">{{ $category->updated_at->format('d M Y') }}</td>
                        <td style="padding:14px 20px;">
                            <div style="display:flex;gap:6px;align-items:center;">
                                ...
                                <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST"
                                      onsubmit="return confirm('Hapus kategori ini secara permanen?');" style="margin:0;">
                                    @csrf @method('DELETE')
        ...
        <div style="padding:16px 20px;border-top:1px solid #f1f5f9;display:flex;align-items:center;flex-wrap:wrap;gap:12px;">
            <p style="font-size:13px;color:#94a3b8;flex:1;">
                Menampilkan <span style="color:#1e293b;font-weight:600;">{{ $categories->firstItem() }}</span>
                – <span style="color:#1e293b;font-weight:600;">{{ $categories->lastItem() }}</span>
                dari <span style="color:#1e293b;font-weight:600;">{{ $categories->total() }}</span> data
            </p>
